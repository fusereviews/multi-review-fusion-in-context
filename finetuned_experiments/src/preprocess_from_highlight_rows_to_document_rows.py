...
def preprocess_from_highlight_rows_to_document_rows(data_json_path: str, train_file_highlight_rows: str, output_file_path: str) -> None:
    """
    ...
    doc_reader = DocReader(data_json_path)
    ...
    highlight_spans_column_names = [col_name for col_name in highlights_df.columns if "highlight_spans" in col_name]
    for col in highlight_spans_column_names:
        highlights_df[col] = highlights_df[col].apply(json.dumps)
...
def main():
    ...
    if config.get('do_preprocess', False):
        preprocess_from_highlight_rows_to_document_rows(config['data_json_path'], config['train_file_highlight_rows'], config['output_file_path'])
